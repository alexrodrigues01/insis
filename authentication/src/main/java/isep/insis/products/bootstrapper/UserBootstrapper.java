...
import isep.insis.products.model.Role;
import isep.insis.products.model.User;
import isep.insis.products.repositories.UserRepository;
...
public class UserBootstrapper implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        if(userRepo.findByUsername("mod@mail.com").isEmpty()) {
            User user12 = new User("mod@mail.com", encoder.encode("mod"),
                    "Rui Mod", "748526327", "Rua doze");
            user12.addAuthority(new Role(Role.Mod));
            userRepo.save(user12);
        }
        if(userRepo.findByUsername("customer@mail.com").isEmpty()) {
            User user12 = new User("customer@mail.com", encoder.encode("customer"),
                    "Rui Customer", "748526328", "Rua doze");
            user12.addAuthority(new Role(Role.Customer));
            userRepo.save(user12);
        }
